use super::{AasStreamSong, MAX_ROM_BYTES, RomSpan};
use crate::gba_bootstrap;

pub struct PreparedAasStreamRom {
    pub bytes: Vec<u8>,
    pub wait_loop: RomSpan,
}

pub(super) fn build(bytes: &[u8], song: &AasStreamSong) -> anyhow::Result<PreparedAasStreamRom> {
    let offset = bytes
        .len()
        .checked_add(3)
        .map(|len| len & !3)
        .filter(|&len| len <= MAX_ROM_BYTES - 512)
        .ok_or_else(|| anyhow::anyhow!("AAS stream bootstrap has no available ROM space"))?;
    ...
    for (register, value) in [
        0,
        64,
        song.encoded_data.canonical_cpu_address - song.native.stream_bank_address,
        song.encoded_nibbles,
    ]
    .into_iter()
    .enumerate()
    {
    ...
    anyhow::ensure!(
        payload.len() <= 512,
        "AAS stream bootstrap exceeds its reserved space"
    );
    ...
    anyhow::ensure!(
        ...
        "AAS stream startup handoff is too small"
    );
    ...
    fn finish(mut self) -> anyhow::Result<Vec<u8>> {
        for &(at, register, value) in &self.literals {
            ...
            anyhow::ensure!(
                ...
                "AAS stream bootstrap literal is out of range"
            );
