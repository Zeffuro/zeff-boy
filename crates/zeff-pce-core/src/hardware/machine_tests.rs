...
#[path = "machine_tests/coalesced_timing.rs"]
mod coalesced_timing;
#[path = "machine_tests/plain_memory_lane.rs"]
mod plain_memory_lane;
#[path = "machine_tests/presented_frame.rs"]
mod presented_frame;
...
fn profiling_is_resettable_and_not_serialized() {
    ...
    let snapshot = source.profiling_snapshot();
    assert!(snapshot.cpu_boundaries != 0);
    assert!(snapshot.bus_plain_hucard_accesses != 0);
    assert!(snapshot.device_materialization_attempts != 0);
    assert!(snapshot.device_materializations_action_finish != 0);
