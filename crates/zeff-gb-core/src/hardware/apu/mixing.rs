...
impl Apu {
    ...
    pub(super) fn pcm12(&self) -> u8 {
        self.square_pcm_output(0, self.ch1_duty_pos)
            | (self.square_pcm_output(1, self.ch2_duty_pos) << 4)
    }

    pub(super) fn pcm34(&self) -> u8 {
        self.ch3_pcm_output() | (self.ch4_pcm_output() << 4)
    }
}
