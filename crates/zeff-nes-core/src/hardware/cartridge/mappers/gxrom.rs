...
impl Mapper for Gxrom {
    ...
    fn read_state(&mut self, r: &mut crate::save_state::StateReader) -> anyhow::Result<()> {
        ...
        crate::save_state::read_chr_state(r, &mut self.chr, "GxROM")?;
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn switches_32k_prg_and_8k_chr_with_bus_conflict_safe_write() {
        let mut prg = prg_banks(&[0x00, 0x22, 0x44, 0x66]);
        prg[0] = 0x11;
        let chr = chr_banks(&[0x00, 0xBB, 0xCC, 0xDD]);

        let mut mapper = Gxrom::new(prg, chr, Mirroring::Horizontal);

        mapper.cpu_write(0x8000, 0x11);
        assert_eq!(mapper.cpu_peek(0x8000), 0x22);
        assert_eq!(mapper.chr_read(0x0000), 0xBB);
    }
    ...
    fn bus_conflict_ands_write_with_rom_byte() {
        ...
        mapper.cpu_write(0x8000, 0x11);
        assert_eq!(mapper.bank_select, 0x11);

        mapper.cpu_write(0x8000, 0x33);
        assert_eq!(mapper.bank_select, 0x22);
        assert_eq!(mapper.cpu_peek(0x8000), 0x44);
        assert_eq!(mapper.chr_read(0x0000), 0xCC);
    }

    #[test]
    fn chr_ram_is_read_write_when_header_has_no_chr_rom() {
        let mut mapper = Gxrom::new(prg_banks(&[0x00]), vec![0; 0x2000], Mirroring::Vertical);

        mapper.chr_write(0x0200, 0x5A);
        assert_eq!(mapper.chr_read(0x0200), 0x5A);
    }

    #[test]
    fn state_roundtrips_bank_and_chr() {
        let mut mapper = Gxrom::new(
            prg_banks(&[0x00, 0x22]),
            chr_banks(&[0x00, 0xBB]),
