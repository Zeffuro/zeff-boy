...
impl Cpu {
    pub fn adc(&mut self, val: u8) {
        ...
        let v = val as u16;
        let c = if self.regs.get_flag(StatusFlags::CARRY) {
            1u16
        } else {
            0
        };
        ...
        self.regs
            .set_flag(StatusFlags::OVERFLOW, (!(a ^ v) & (a ^ sum)) & 0x80 != 0);
        self.regs.a = result;
    ...
    pub fn bit_test(&mut self, val: u8) {
        self.regs
            .set_flag(StatusFlags::ZERO, self.regs.a & val == 0);
    ...
    pub fn rol_acc(&mut self) {
        let old = self.regs.a;
        let carry_in = if self.regs.get_flag(StatusFlags::CARRY) {
            1
        } else {
            0
        };
