...
use super::window_geometry::{
    PRINTER_DEFAULT_SIZE, PRINTER_MIN_SIZE, restored_position, restored_size,
};
...
pub(super) struct PrinterWindow {
    window: Arc<Window>,
    gpu: GpuContext,
    egui: EguiRenderer,
...
impl PrinterWindow {
    pub(super) fn new(
        ...
        let size = restored_size(
            settings.ui.printer_window_size,
            PRINTER_MIN_SIZE,
            PRINTER_DEFAULT_SIZE,
        );
        let mut attrs = WindowAttributes::default()
            .with_title(format!(
                "zeff-boy Game Boy Printer v{}",
                env!("CARGO_PKG_VERSION")
            ))
            .with_inner_size(size)
            .with_min_inner_size(PhysicalSize::new(PRINTER_MIN_SIZE[0], PRINTER_MIN_SIZE[1]));
        if let Some(position) =
            restored_position(event_loop, settings.ui.printer_window_position, size)
        {
        ...
        window.set_maximized(settings.ui.printer_window_maximized);
    ...
    pub(super) fn render(&mut self, ctx: PrinterRenderContext<'_>) -> Result<(), FrameError> {
        ...
        egui::CentralPanel::default()
            ...
            .show(&mut root_ui, |ui| {
                crate::debug::draw_printer_viewer_content(ui, ctx.state);
            });
