use super::*;

#[test]
fn parse_dat_entries_extracts_crc_title_and_rom_name() {
    let dat = r#"
        game (
            name "Example Red Version (USA, Europe)"
            rom ( name "Example Red Version (USA, Europe) (SGB Enhanced).gb" size 524288 crc D7037C83 md5 0 sha1 0 )
        )
    "#;

    let entries = parse_dat_entries(dat, LibretroPlatform::Gb);
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0].crc32, 0xD7037C83);
    assert_eq!(entries[0].title, "Example Red Version (USA, Europe)");
    assert_eq!(
        entries[0].rom_name,
        "Example Red Version (USA, Europe) (SGB Enhanced).gb"
    );
    assert_eq!(entries[0].platform, LibretroPlatform::Gb);
}

#[test]
fn parse_dat_entries_nes() {
    let dat = r#"
        game (
            name "Example Platformer (World)"
            rom ( name "Example Platformer (World).nes" size 40976 crc 3337EC46 md5 0 sha1 0 )
        )
    "#;

    let entries = parse_dat_entries(dat, LibretroPlatform::Nes);
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0].crc32, 0x3337EC46);
    assert_eq!(entries[0].title, "Example Platformer (World)");
    assert_eq!(entries[0].rom_name, "Example Platformer (World).nes");
    assert_eq!(entries[0].platform, LibretroPlatform::Nes);
}

#[test]
fn parse_dat_entries_master_system() {
    let dat = r#"
        game (
            name "Example Fast Game (USA, Europe)"
            rom ( name "Example Fast Game (USA, Europe).sms" size 262144 crc B519E833 md5 0 sha1 0 )
        )
    "#;

    let entries = parse_dat_entries(dat, LibretroPlatform::MasterSystem);
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0].crc32, 0xB519E833);
    assert_eq!(entries[0].title, "Example Fast Game (USA, Europe)");
    assert_eq!(entries[0].rom_name, "Example Fast Game (USA, Europe).sms");
    assert_eq!(entries[0].platform, LibretroPlatform::MasterSystem);
}

#[test]
fn build_cheat_search_hints_prefers_metadata_aliases() {
    let meta = RomMetadata {
        crc32: 0,
        title: "Example Red Version (USA, Europe)".to_string(),
        rom_name: "Example Red Version (USA, Europe) (SGB Enhanced).gb".to_string(),
        platform: LibretroPlatform::Gb,
    };

    let hints = build_cheat_search_hints("EXAMPLE RED", Some(&meta));
    assert!(hints.iter().any(|h| h.contains("Example Red Version")));
    assert!(hints.iter().any(|h| h == "example red version usa europe"));
}

#[test]
fn build_cheat_search_hints_nes() {
    let meta = RomMetadata {
        crc32: 0,
        title: "Example Platformer (World)".to_string(),
        rom_name: "Example Platformer (World).nes".to_string(),
        platform: LibretroPlatform::Nes,
    };

    let hints = build_cheat_search_hints("EXAMPLE PLATFORMER", Some(&meta));
    assert!(hints.iter().any(|h| h.contains("Example Platformer")));
}

#[test]
fn serialize_roundtrip_preserves_entries() {
    let entries = vec![
        RomMetadata {
            crc32: 0x1234ABCD,
            title: "Test Game".to_string(),
            rom_name: "Test Game.gb".to_string(),
            platform: LibretroPlatform::Gb,
        },
        RomMetadata {
            crc32: 0xDEADBEEF,
            title: "NES Test".to_string(),
            rom_name: "NES Test.nes".to_string(),
            platform: LibretroPlatform::Nes,
        },
        RomMetadata {
            crc32: 0xB519E833,
            title: "Example Fast Game".to_string(),
            rom_name: "Example Fast Game.sms".to_string(),
            platform: LibretroPlatform::MasterSystem,
        },
        RomMetadata {
            crc32: 0xCAFEBABE,
            title: "Game Gear Test".to_string(),
            rom_name: "Game Gear Test.gg".to_string(),
            platform: LibretroPlatform::GameGear,
        },
    ];

    let bytes = serialize_entries(&entries).unwrap();
    let parsed = deserialize_entries(&bytes).unwrap();
    assert_eq!(parsed.len(), 4);
    assert_eq!(parsed[0].crc32, 0x1234ABCD);
    assert_eq!(parsed[0].title, "Test Game");
    assert_eq!(parsed[0].rom_name, "Test Game.gb");
    assert_eq!(parsed[0].platform, LibretroPlatform::Gb);
    assert_eq!(parsed[1].crc32, 0xDEADBEEF);
    assert_eq!(parsed[1].title, "NES Test");
    assert_eq!(parsed[1].rom_name, "NES Test.nes");
    assert_eq!(parsed[1].platform, LibretroPlatform::Nes);
    assert_eq!(parsed[2].platform, LibretroPlatform::MasterSystem);
    assert_eq!(parsed[3].platform, LibretroPlatform::GameGear);
}

#[test]
fn lookup_cached_filters_by_platform() {
    let entries = vec![RomMetadata {
        crc32: 0xAABBCCDD,
        title: "Some GB Game".to_string(),
        rom_name: "Some GB Game.gb".to_string(),
        platform: LibretroPlatform::Gb,
    }];

    let bytes = serialize_entries(&entries).unwrap();
    let parsed = deserialize_entries(&bytes).unwrap();
    let index = build_index(parsed);

    let gb_hit = index.by_crc.get(&0xAABBCCDD);
    assert!(gb_hit.is_some());
    assert_eq!(gb_hit.unwrap().platform, LibretroPlatform::Gb);

    let nes_hit = index
        .by_crc
        .get(&0xAABBCCDD)
        .filter(|e| e.platform == LibretroPlatform::Nes);
    assert!(nes_hit.is_none());
}
