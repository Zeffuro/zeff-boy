...
use super::{MetadataRefreshStats, RomMetadata};
...
pub(super) struct MetadataIndex {
    pub(super) by_crc: HashMap<u32, RomMetadata>,
}
...
pub(super) fn parse_dat_entries(dat: &str, platform: LibretroPlatform) -> Vec<RomMetadata> {
...
pub(super) fn serialize_entries(entries: &[RomMetadata]) -> anyhow::Result<Vec<u8>> {
...
pub(super) fn deserialize_entries(bytes: &[u8]) -> anyhow::Result<Vec<RomMetadata>> {
...
pub(super) fn build_index(entries: Vec<RomMetadata>) -> MetadataIndex {
