...
pub(crate) fn save_file_path_for_rom(path: &Path) -> PathBuf {
    path.with_extension("sav")
}

pub(crate) fn load_save_file(path: &Path) -> std::io::Result<Vec<u8>> {
    fs::read(path)
}

pub(crate) fn write_save_file(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
