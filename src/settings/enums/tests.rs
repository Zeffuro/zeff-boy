...
fn vsync_on_always_fifo() {
    ...
    assert_eq!(
        VsyncMode::On.to_present_mode(&caps),
        wgpu::PresentMode::Fifo
    );
}
...
fn vsync_off_prefers_immediate() {
    ...
    assert_eq!(
        VsyncMode::Off.to_present_mode(&caps),
        wgpu::PresentMode::Immediate
    );
}
...
fn vsync_adaptive_prefers_auto_vsync() {
    let caps = vec![wgpu::PresentMode::Fifo, wgpu::PresentMode::AutoVsync];
    assert_eq!(
        VsyncMode::Adaptive.to_present_mode(&caps),
