...
const WS_CYCLES_PER_FRAME: u64 =
    zeff_ws_core::hardware::constants::CYCLES_PER_FRAME as u64;
const WATERMARK_INTERVAL_CYCLES: u64 = WS_CYCLES_PER_FRAME / 2;
const MAX_REMOTE_LEAD_CYCLES: u64 = WS_CYCLES_PER_FRAME * 2;
const REMOTE_RX_DELAY_CYCLES: u64 = MAX_REMOTE_LEAD_CYCLES;
...
impl<T: LinkTransport> WonderSwanRemoteLink<T> {
    ...
    fn deliver_due_events(&mut self, emulator: &mut WonderSwanEmulator) {
        ...
        while self
            .inbound_events
            .front()
            .is_some_and(|event| self.delivery_cycle(*event) <= local_cycle)
        {
            if delivered >= MAX_RX_DELIVERIES_PER_POLL {
                self.trace(format!(
                    ...
                    self.inbound_events
                        .iter()
                        .take_while(|event| self.delivery_cycle(**event) <= local_cycle)
                        .count(),
                    self.inbound_events.len()
                ));
                break;
            ...
            let delivery_cycle = self.delivery_cycle(event);
            let late_by = local_cycle.saturating_sub(delivery_cycle);
            if late_by != 0 {
                self.rx_delivery_cycle_offset =
                    self.rx_delivery_cycle_offset.saturating_add(late_by);
            }
            delivered += 1;
            self.trace(format!(
                "deliver rx local_session_cycle={} tx_session_cycle={} delivery_session_cycle={} late_by={} byte={:02X} queue_after={} rx_delay_offset={}",
                local_cycle,
                event.completed_cycle,
                delivery_cycle,
                late_by,
        ...
        if self.inbound_events.is_empty() {
            self.rx_delivery_cycle_offset = 0;
        }
    }
    ...
    fn delivery_cycle(&self, event: WonderSwanLinkEvent) -> u64 {
        event
            .completed_cycle
            .saturating_add(REMOTE_RX_DELAY_CYCLES)
            .saturating_add(self.rx_delivery_cycle_offset)
    }
