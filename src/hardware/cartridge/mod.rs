...
impl Cartridge {
    ...
    pub(crate) fn set_rumble_flag(&mut self, has_rumble: bool) {
        if let Cartridge::Mbc5(c) = self {
            c.set_has_rumble(has_rumble);
        }
    }

    pub(crate) fn set_mbc7_tilt(&mut self, x: f32, y: f32) {
    ...
    pub(crate) fn mbc_ram_bytes(&self) -> &[u8] {
        match self {
    ...
    pub(crate) fn bess_mbc_writes(&self) -> Vec<(u16, u8)> {
        match self {
    ...
    pub(crate) fn bess_rtc_data(&self) -> Option<([u8; 5], [u8; 5])> {
        match self {
