...
impl RomHeader {
    pub(crate) fn from_rom(rom: &[u8]) -> Result<Self> {
        ...
        let cgb_flag = *rom.get(header_constants::CGB_FLAG_IDX)
            .ok_or_else(|| anyhow!("CGB flag missing"))?;
        let sgb_flag = *rom.get(header_constants::SGB_FLAG_IDX)
            .ok_or_else(|| anyhow!("SGB flag missing"))?;

        let is_cgb_compatible = cgb_flag == header_constants::CGB_FLAG_COMPATIBLE || cgb_flag == header_constants::CGB_FLAG_EXCLUSIVE;
        ...
        let (title_range, manufacturer_range) = if is_cgb_compatible {
            ...
            (header_constants::TITLE_START..header_constants::TITLE_END_NEW,
             header_constants::MANUFACTURER_CODE_START..header_constants::MANUFACTURER_CODE_START)
        } else {
        ...
        let manufacturer_code = if is_cgb_compatible {
            let code_bytes = rom.get(manufacturer_range.clone())
                .ok_or_else(|| anyhow!("ROM too small for manufacturer code: expected {:?} bytes", manufacturer_range))?;
            let code_string = String::from_utf8_lossy(code_bytes).trim_end_matches('\0').to_string();
    ...
    pub(crate) fn display_info(&self) {
        ...
        log::info!("CGB Flag: {:#04X} (Compatible: {}, Exclusive: {})", self.cgb_flag, self.is_cgb_compatible, self.is_cgb_exclusive);
        log::info!("SGB Flag: {:#04X} (Supported: {})", self.sgb_flag, self.is_sgb_supported);
        log::info!("Destination: {:#04X} (Overseas: {})", self.destination, self.is_overseas);
        log::info!("Licensee (old): {:#04X}", self.old_licensee_code);
        log::info!("Licensee (new): {}", self.new_licensee_code.as_deref().unwrap_or("N/A"));
