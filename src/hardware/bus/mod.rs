...
impl Bus {
    pub(crate) fn joypad_p1(&self) -> u8 {
        self.io.joypad.read()
    }

    pub(crate) fn write_joypad_p1(&mut self, value: u8) {
        self.io.joypad.write(value);
    }

    pub(crate) fn apply_joypad_pressed_masks(&mut self, buttons_pressed: u8, dpad_pressed: u8) -> bool {
        self.io
            .joypad
            .apply_pressed_masks(buttons_pressed, dpad_pressed)
    }

    pub(crate) fn apu_sample_rate(&self) -> u32 {
        self.io.apu.sample_rate
    }

    pub(crate) fn set_apu_sample_rate(&mut self, sample_rate: u32) {
    ...
    pub(crate) fn apu_drain_samples_into(&mut self, target: &mut Vec<f32>) {
        self.io.apu.drain_samples_into(target);
    }

    pub(crate) fn apu_drain_samples(&mut self) -> Vec<f32> {
        self.io.apu.drain_samples()
    }

    pub(crate) fn apu_channel_snapshot(&self) -> crate::hardware::apu::ApuChannelSnapshot {
    ...
    pub(crate) fn apu_regs_snapshot(&self) -> [u8; 0x17] {
        self.io.apu.regs_snapshot()
    }

    pub(crate) fn apu_wave_ram_snapshot(&self) -> [u8; 0x10] {
        self.io.apu.wave_ram_snapshot()
    }

    pub(crate) fn apu_nr52_raw(&self) -> u8 {
        self.io.apu.nr52_raw()
    }

    pub(crate) fn apu_channel_debug_samples_ordered(&self, channel: usize) -> [f32; 512] {
        self.io.apu.channel_debug_samples_ordered(channel)
    }

    pub(crate) fn apu_master_debug_samples_ordered(&self) -> [f32; 512] {
        self.io.apu.master_debug_samples_ordered()
    }

    pub(crate) fn apu_channel_mutes(&self) -> [bool; 4] {
        self.io.apu.channel_mutes()
    }

    pub(crate) fn apply_bess_apu_io(&mut self, io_regs: &[u8]) {
    ...
    pub(crate) fn ppu_framebuffer(&self) -> &[u8] {
        &self.io.ppu.framebuffer
    }

    pub(crate) fn ppu_lcdc(&self) -> u8 {
        self.io.ppu.lcdc
    }

    pub(crate) fn ppu_stat(&self) -> u8 {
        self.io.ppu.stat
    }

    pub(crate) fn ppu_scy(&self) -> u8 {
        self.io.ppu.scy
    }

    pub(crate) fn ppu_scx(&self) -> u8 {
        self.io.ppu.scx
    }

    pub(crate) fn ppu_ly(&self) -> u8 {
        self.io.ppu.ly
    }

    pub(crate) fn ppu_lyc(&self) -> u8 {
        self.io.ppu.lyc
    }

    pub(crate) fn ppu_wy(&self) -> u8 {
        self.io.ppu.wy
    }

    pub(crate) fn ppu_wx(&self) -> u8 {
        self.io.ppu.wx
    }

    pub(crate) fn ppu_bgp(&self) -> u8 {
        self.io.ppu.bgp
    }

    pub(crate) fn ppu_obp0(&self) -> u8 {
        self.io.ppu.obp0
    }

    pub(crate) fn ppu_obp1(&self) -> u8 {
        self.io.ppu.obp1
    }

    pub(crate) fn ppu_cycles(&self) -> u64 {
        self.io.ppu.cycles
    }

    pub(crate) fn ppu_cgb_mode(&self) -> bool {
        self.io.ppu.cgb_mode
    }

    pub(crate) fn ppu_bg_palette_ram_snapshot(&self) -> [u8; 0x40] {
        self.io.ppu.bg_palette_ram
    }

    pub(crate) fn ppu_obj_palette_ram_snapshot(&self) -> [u8; 0x40] {
        self.io.ppu.obj_palette_ram
    }

    pub(crate) fn ppu_bg_palette_ram(&self) -> &[u8; 0x40] {
        &self.io.ppu.bg_palette_ram
    }

    pub(crate) fn ppu_obj_palette_ram(&self) -> &[u8; 0x40] {
        &self.io.ppu.obj_palette_ram
    }

    pub(crate) fn ppu_bg_palette_ram_mut(&mut self) -> &mut [u8; 0x40] {
        &mut self.io.ppu.bg_palette_ram
    }

    pub(crate) fn ppu_obj_palette_ram_mut(&mut self) -> &mut [u8; 0x40] {
        &mut self.io.ppu.obj_palette_ram
    }

    pub(crate) fn ppu_color_correction(&self) -> crate::settings::ColorCorrection {
        self.io.ppu.color_correction
    }

    pub(crate) fn ppu_color_correction_matrix(&self) -> [f32; 9] {
        self.io.ppu.color_correction_matrix
    }

    pub(crate) fn set_ppu_color_correction(
        &mut self,
        color_correction: crate::settings::ColorCorrection,
    ) {
        self.io.ppu.color_correction = color_correction;
    }

    pub(crate) fn set_ppu_color_correction_matrix(&mut self, matrix: [f32; 9]) {
        self.io.ppu.color_correction_matrix = matrix;
    }

    pub(crate) fn set_ppu_debug_flags(&mut self, bg: bool, window: bool, sprites: bool) {
        self.io.ppu.debug_flags.bg = bg;
        self.io.ppu.debug_flags.window = window;
        self.io.ppu.debug_flags.sprites = sprites;
    }

    pub(crate) fn set_ppu_sgb_mode(&mut self, enabled: bool) {
        self.io.ppu.set_sgb_mode(enabled);
    }

    pub(crate) fn apply_bess_ppu_registers(&mut self, io: &[u8], is_cgb: bool) {
    ...
    pub(crate) fn timer_div(&self) -> u8 {
        self.io.timer.div()
    }

    pub(crate) fn timer_tima(&self) -> u8 {
        self.io.timer.tima()
    }

    pub(crate) fn timer_tma(&self) -> u8 {
        self.io.timer.tma()
    }
    ...
    pub(crate) fn sync_timer_serial_mode(&mut self) {
        self.io.timer.set_mode(self.hardware_mode);
        self.io.serial.set_mode(self.hardware_mode);
    }

    pub(crate) fn apply_bess_timer_serial_registers(&mut self, io: &[u8], mode: HardwareMode) {
