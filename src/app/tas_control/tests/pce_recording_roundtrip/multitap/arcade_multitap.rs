...
fn fixture(label: &str) -> Fixture {
    ...
    let fill = label.bytes().fold(0xDC, u8::wrapping_add);
...
#[test]
fn linked_app_records_arcade_multitap_and_supports_restore_and_keep() {
    for keep in [false, true] {
        let fixture = fixture(if keep {
            "tas-pce-cd-arcade-multitap-live-keep"
        } else {
            "tas-pce-cd-arcade-multitap-live-restore"
        });
        let _catalogs = (&fixture.arcade_catalog, &fixture.controller_catalog);
        let generation = if keep { 136 } else { 135 };
...
#[test]
fn arcade_multitap_repair_reloads_and_restores_running_worker() {
    let fixture = fixture("tas-pce-cd-arcade-multitap-repair");
    let _catalogs = (&fixture.arcade_catalog, &fixture.controller_catalog);
    ...
    let mut app = app_for_fixture(&fixture, 137);
    assert_eq!(
