...
#[test]
fn musyx_graph_assets_and_recordings_preserve_the_revalidated_selector() -> Result<()> {
    let cancel = AtomicBool::new(false);
    let input = input(System::Gb, zeff_audio_discovery::gb_musyx::synthetic_rom());
    let mut manifest = input.analyze(Default::default(), &cancel);
    let id = SongId::GbMusyx(1);
    ...
    for span in &manifest.scan.gb_musyx_songs[1].mapped_spans {
        let name = format!(
            "source/{:08x}-{:08x}.bin",
    ...
    assert_eq!(metadata["selection"]["engine"], "gb_musyx");
    assert_eq!(
        metadata["selection"]["song"],
        serde_json::to_value(&manifest.scan.gb_musyx_songs[1])?
    );
    let mut session =
    ...
    manifest.scan.gb_musyx_songs[1].mapped_spans[0].effective_offset += 1;
