...
pub(crate) fn collect_gba_snapshot(
    ...
    if snapshot.show_apu_viewer {
        data.apu_debug = Some(gba_apu_snapshot(emu));
    }
...
fn gba_apu_snapshot(emu: &zeff_gba_core::emulator::Emulator) -> crate::debug::ApuDebugInfo {
            ...
            crate::debug::DebugSection {
                ...
                lines: vec![
                    ...
                    format!(
                        ...
                        on_off(ppu.debug_flags.bg),
                        on_off(ppu.debug_flags.bg_layers[0]),
                        on_off(ppu.debug_flags.bg_layers[1]),
                        on_off(ppu.debug_flags.bg_layers[2]),
                        on_off(ppu.debug_flags.bg_layers[3]),
                        on_off(ppu.debug_flags.window),
                        on_off(ppu.debug_flags.sprites)
                    ),
                    format!(
