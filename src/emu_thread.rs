...
impl EmuThread {
    ...
    fn handle_step_frames(
        ...
        if let Some(emu) = backend.gb_mut() {
            ...
            emu.set_apu_debug_capture_enabled(input.apu_capture_enabled);
            if !uncapped_mode {
                emu.set_apu_sample_generation_enabled(!input.skip_audio);
            }
