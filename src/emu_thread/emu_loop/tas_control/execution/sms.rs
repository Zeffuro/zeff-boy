...
pub(in crate::emu_thread::emu_loop::tas_control) fn execute_direct_sms_tas(
    backend: &mut EmuBackend,
    runtime_fault: &mut WorkerRuntimeFault,
    request: &TasExecutionRequest,
) -> Result<TasExecutionResult, Rejected> {
    validate_sms_inputs(&request.input_prefix).map_err(|_| Rejected::InvalidInput)?;
    ...
    restore_direct_sms_state(backend, &request.start_state_bytes)?;
    ...
    capture_direct_sms_candidate(backend, expected_frame).map(|(frame_count, state_sha256)| {
        TasExecutionResult {
...
pub(in crate::emu_thread::emu_loop::tas_control) fn validate_sms_input(
    input: TasInputFrame,
) -> Result<(), ()> {
...
pub(in crate::emu_thread::emu_loop::tas_control) fn validate_sms_inputs(
    inputs: &[TasInputFrame],
) -> Result<(), ()> {
    inputs.iter().copied().try_for_each(validate_sms_input)
}

pub(in crate::emu_thread::emu_loop::tas_control) fn validate_direct_sms_start_state(
    backend: &EmuBackend,
    state: &[u8],
) -> Result<(), Rejected> {
...
pub(in crate::emu_thread::emu_loop::tas_control) fn restore_direct_sms_state(
    backend: &mut EmuBackend,
    state: &[u8],
) -> Result<(), Rejected> {
    validate_direct_sms_start_state(backend, state)?;
    let projection = crate::emu_backend::loader::validate_direct_sms_tas_state(backend, state)
...
pub(in crate::emu_thread::emu_loop::tas_control) fn capture_direct_sms_candidate(
    backend: &EmuBackend,
    expected_frame: u64,
    ...
    let state = backend
        .encode_state_bytes()
        .map_err(|_| Rejected::StateCaptureFailed)?;
    validate_direct_sms_start_state(backend, &state).map_err(|_| Rejected::StateCaptureFailed)?;
