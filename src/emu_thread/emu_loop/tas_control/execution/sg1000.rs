...
pub(in crate::emu_thread::emu_loop::tas_control) fn execute_direct_sg1000_tas(
    backend: &mut EmuBackend,
    runtime_fault: &mut WorkerRuntimeFault,
    request: &TasExecutionRequest,
) -> Result<TasExecutionResult, Rejected> {
    validate_sg1000_inputs(&request.input_prefix).map_err(|_| Rejected::InvalidInput)?;
    ...
    restore_direct_sg1000_state(backend, &request.start_state_bytes)?;
    ...
    capture_direct_sg1000_candidate(backend, expected_frame).map(|(frame_count, state_sha256)| {
        TasExecutionResult {
...
pub(in crate::emu_thread::emu_loop::tas_control) fn validate_sg1000_input(
    input: TasInputFrame,
) -> Result<(), ()> {
...
pub(in crate::emu_thread::emu_loop::tas_control) fn validate_sg1000_inputs(
    inputs: &[TasInputFrame],
) -> Result<(), ()> {
    inputs.iter().copied().try_for_each(validate_sg1000_input)
}

pub(in crate::emu_thread::emu_loop::tas_control) fn validate_direct_sg1000_start_state(
    backend: &EmuBackend,
    state: &[u8],
) -> Result<(), Rejected> {
    crate::emu_backend::loader::validate_direct_sg1000_tas_execution_runtime(backend, false)
    ...
    if inspection.rom_sha256 != sega8.emu.rom_hash()
        ...
        || inspection.console_region != zeff_sega8_core::hardware::region::Sega8Region::Japanese
        || inspection.boot_rom_enabled
        || inspection.type_b_ram_extension
    {
...
pub(in crate::emu_thread::emu_loop::tas_control) fn restore_direct_sg1000_state(
    backend: &mut EmuBackend,
    state: &[u8],
) -> Result<(), Rejected> {
    validate_direct_sg1000_start_state(backend, state)?;
    let projection = crate::emu_backend::loader::validate_direct_sg1000_tas_state(backend, state)
...
pub(in crate::emu_thread::emu_loop::tas_control) fn capture_direct_sg1000_candidate(
    backend: &EmuBackend,
    expected_frame: u64,
