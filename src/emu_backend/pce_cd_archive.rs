...
mod tests {
    ...
    use zeff_pce_core::hardware::CDROM2_REGISTER_START;
    ...
    struct CdTraceState {
        phase: zeff_pce_core::hardware::CdScsiPhase,
        audio: zeff_pce_core::hardware::CdAudioStatus,
        irq2: zeff_pce_core::hardware::cpu::LineLevel,
        registers: [u8; 8],
        command_count: usize,
    }

    fn cd_trace_state(backend: &EmuBackend) -> CdTraceState {
        let EmuBackend::Pce(pce) = backend else {
            panic!("CD trace requires PCE backend");
        };
        let cd = pce.cdrom2().unwrap();
        CdTraceState {
            phase: cd.phase(),
            audio: cd.audio_status(),
            irq2: cd.irq2_level(),
            registers: [2, 3, 9, 11, 12, 13, 14, 15].map(|offset| {
                let value = cd.peek_physical(CDROM2_REGISTER_START + offset).unwrap();
                match offset {
                    2 => value & 0x7C,
                    3 => value & 0x0C,
                    _ => value,
            ...
            command_count: cd.command_trace().len(),
        }
    }

    fn trace_local_cd_boundaries(
        backend: &mut EmuBackend,
        frame_limit: u64,
        run_frames: &BTreeSet<u64>,
    ) {
        let mut frame = 1_u64;
        let mut state = cd_trace_state(backend);
        let mut audio = Vec::new();
        let mut pcm_capture = Vec::new();
        backend.set_input(u8::from(run_frames.contains(&frame)) << 3, 0);
        for boundary in 0..100_000_000_u64 {
            let (pc, published) = match backend {
                EmuBackend::Pce(pce) => {
                    let pc = pce.debug_cpu_snapshot().registers().pc;
                    (pc, pce.debug_step_boundary().unwrap())
                }
                _ => unreachable!(),
            };
            let next = cd_trace_state(backend);
            if next != state {
                let command = if next.command_count != state.command_count {
                    let EmuBackend::Pce(pce) = &*backend else {
                        unreachable!()
                    };
                    pce.cdrom2()
                        ...
                        .command_trace()
                        .back()
                        .map(|command| command.bytes().to_vec())
                } else {
                    None
                };
                println!(
                    "frame={frame} boundary={boundary} pc={pc:04x} state={next:?} command={command:02x?}"
                );
                state = next;
            }
            if published != 0 {
                backend.drain_audio_samples_into(&mut audio);
                if state.registers[4] & 0x08 != 0 {
                    pcm_capture.extend(audio.iter().flat_map(|sample| sample.to_le_bytes()));
                }
                if state.registers[4] & 0x08 != 0
                    && (frame == 1 || frame.is_multiple_of(30) || published > 1)
                {
                    ...
                    println!(
                        "frame_audio frame={frame} stereo_samples={} rms={rms:.6} peak={peak:.6}",
                        audio.len()
                    );
                }
                audio.clear();
        ...
        let cd = pce.cdrom2().unwrap();
        if let Ok(path) = std::env::var("ZEFF_PCE_CD_SMOKE_PCM_CAPTURE") {
            std::fs::write(path, &pcm_capture).unwrap();
        }
        println!(
            "trace_complete frames={} adpcm_nonzero={} pcm_bytes={} commands={:?}",
            frame.saturating_sub(1),
            cd.adpcm_ram().iter().filter(|byte| **byte != 0).count(),
            pcm_capture.len(),
            cd.command_trace()
                .iter()
                .map(|command| command.bytes().to_vec())
    ...
    #[test]
    #[ignore = "requires local PCE-CD 7z and matching System Card paths"]
    fn local_package_async_preparation_smoke() {
        let archive = PathBuf::from(std::env::var("ZEFF_PCE_CD_7Z_SMOKE").unwrap());
        let system_card_path = PathBuf::from(std::env::var("ZEFF_PCE_CD_SYSTEM_CARD").unwrap());
        let system_card: &'static [u8] =
            Box::leak(std::fs::read(system_card_path).unwrap().into_boxed_slice());
        let console_wiring =
            std::env::var("ZEFF_PCE_CD_SMOKE_CONSOLE")
                .ok()
                .map(|value| match value.as_str() {
                    "pce" => zeff_pce_core::hardware::PceConsoleWiring::PcEngine,
                    "tg16" => zeff_pce_core::hardware::PceConsoleWiring::TurboGrafx16,
                    _ => panic!("ZEFF_PCE_CD_SMOKE_CONSOLE must be pce or tg16"),
                });
        let cancelled = AtomicBool::new(true);
        let cancelled_progress = PceCdPackageProgress::default();
        assert_eq!(
            crate::emu_backend::loader::prepare_pce_cd_7z_backend(
                &archive,
                None,
                &BackendLoadConfig {
                    pce_cd_system_card_override: Some(system_card),
                    pce_console_wiring: console_wiring,
                    ..BackendLoadConfig::default()
                },
                &cancelled,
                &cancelled_progress,
            ...
            .err()
            .and_then(|error| error.downcast::<PceCdLoadError>().ok()),
            Some(PceCdLoadError::ArchiveCancelled)
        );
        ...
        assert!(
            cue_path
                .extension()
                .is_some_and(|extension| extension == "cue")
        );
        assert_eq!(progress.phase(), PceCdPackageLoadPhase::Complete);
        assert_eq!(progress.completed_bytes(), progress.total_bytes());

        let mut backend = loaded.backend;
        let frame_limit = std::env::var("ZEFF_PCE_CD_SMOKE_FRAMES")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(600);
        let input_frames = |name: &str, fallback: &str| {
            std::env::var(name)
        ...
        let run_frames = input_frames("ZEFF_PCE_CD_SMOKE_RUN_FRAMES", "20,21");
        let i_frames = input_frames("ZEFF_PCE_CD_SMOKE_I_FRAMES", "");
        let ii_frames = input_frames("ZEFF_PCE_CD_SMOKE_II_FRAMES", "");
        let select_frames = input_frames("ZEFF_PCE_CD_SMOKE_SELECT_FRAMES", "");
        if std::env::var_os("ZEFF_PCE_CD_SMOKE_TRACE").is_some() {
            trace_local_cd_boundaries(&mut backend, frame_limit, &run_frames);
            return;
        }
        let right_range = std::env::var("ZEFF_PCE_CD_SMOKE_RIGHT_RANGE")
            .ok()
            .and_then(|value| {
                value
                    .split_once('-')
        ...
        let mut audio_sum_squares = 0.0_f64;
        let mut audio_peak = 0.0_f32;
        for frame in 1..=frame_limit {
            let buttons = u8::from(i_frames.contains(&frame))
                | (u8::from(ii_frames.contains(&frame)) << 1)
                | (u8::from(select_frames.contains(&frame)) << 2)
                | (u8::from(run_frames.contains(&frame)) << 3);
            let dpad =
                u8::from(right_range.is_some_and(|(start, end)| (start..=end).contains(&frame)));
            ...
            backend.step_frame();
            assert_eq!(backend.take_runtime_fault(), None, "fault at frame {frame}");
            backend.drain_audio_samples_into(&mut audio);
            audio_sample_count += audio.len() as u64;
            for &sample in &audio {
        ...
        assert_eq!(backend.frame_count(), frame_limit);
        assert!(visible);
        if let Ok(steps) = std::env::var("ZEFF_PCE_CD_SMOKE_PC_HISTOGRAM_STEPS") {
            let steps = steps.parse::<usize>().unwrap();
            let mut pcs = BTreeMap::<u16, usize>::new();
            for _ in 0..steps {
                let EmuBackend::Pce(pce) = &mut backend else {
                    unreachable!()
                };
                pce.debug_step_boundary().unwrap();
                *pcs.entry(pce.debug_cpu_snapshot().registers().pc)
                    .or_default() += 1;
            }
            let mut top = pcs.into_iter().collect::<Vec<_>>();
            top.sort_unstable_by_key(|(_, count)| std::cmp::Reverse(*count));
            println!("post_smoke_top_pcs={:04x?}", &top[..top.len().min(24)]);
        }
        if let EmuBackend::Pce(pce) = &backend {
            let cpu = pce.debug_cpu_snapshot();
            println!(
                "board={:?} cpu={:?} mpr={:02x?}",
                pce.hucard_board(),
                cpu.registers(),
            ...
            println!(
                "cpu_bytes={:02x?}",
                (0..16)
                    .map(|offset| pce.debug_peek8(u32::from(pc.wrapping_add(offset))))
                    .collect::<Vec<_>>()
            );
            let frame = pce.debug_presented_frame();
            let widths = frame
                .rows()
                .iter()
                .filter(|row| row.is_active())
                .map(|row| (row.active_x_origin(), row.active_width()))
                .collect::<BTreeSet<_>>();
            let repeated_edge_rows = frame
                .rows()
                .iter()
                .enumerate()
                .filter(|(_, row)| row.is_active() && row.active_width() >= 16)
                .filter(|(line, row)| {
                    let width = usize::from(row.active_width());
                    let start = line * zeff_pce_core::hardware::PCE_ACTIVE_FRAME_WIDTH * 4;
                    let rgba = frame.rgba();
                    rgba[start..start + 8 * 4] == rgba[start + (width - 8) * 4..start + width * 4]
                })
                .count();
            println!("active_spans={widths:?} repeated_edge_rows={repeated_edge_rows}");
            println!("vdc_display={:04x?}", pce.debug_vdc_display_registers());
            println!("controller={:?}", pce.debug_controller_device());
            if let Some(cdrom2) = pce.cdrom2() {
                println!(
                    "cd_phase={:?} cd_audio={:?} cd_commands={:?}",
                    cdrom2.phase(),
                    cdrom2.audio_status(),
                    cdrom2
                        .command_trace()
            ...
            let bat = pce.debug_vdc_edge_bat_words();
            println!(
                "bat_col43_eq_col0={} bat_col43_eq_col42={}",
                bat.iter().filter(|(first, _, last)| first == last).count(),
                bat.iter().filter(|(_, prior, last)| prior == last).count(),
            );
        }
        if let Ok(path) = std::env::var("ZEFF_PCE_CD_SMOKE_CAPTURE") {
            let mut ppm = format!(
                "P6\n{} {}\n255\n",
                super::super::pce::PCE_PRESENTED_WIDTH,
                super::super::pce::PCE_PRESENTED_HEIGHT
            )
            .into_bytes();
            ppm.extend(
                backend
        ...
        println!(
            "cue={} content_sha256={} progress={}/{} frames={}",
            cue_path.display(),
            backend
                .rom_hash()
                .iter()
        ...
        println!(
            "audio_samples={audio_sample_count} audio_rms={:.6} audio_peak={audio_peak:.6}",
            (audio_sum_squares / audio_sample_count.max(1) as f64).sqrt()
        );
    }

    #[test]
    #[ignore = "requires exact local Lemmings PCE-CD package"]
    fn local_index_zero_package_diagnostic() {
        use zeff_pce_core::hardware::{
            CdRom2, PROVISIONAL_CDROM2_NEXT_REQUEST_TICKS, PROVISIONAL_CDROM2_PHASE_TICKS,
            PROVISIONAL_CDROM2_SELECTION_TICKS,
        };

        fn control(cd: &mut CdRom2, value: u8) {
            cd.write_physical(CDROM2_REGISTER_START + 2, value);
        }
        ...
        let archive = PathBuf::from(std::env::var("ZEFF_PCE_CD_INDEX00_SMOKE").unwrap());
        let loaded = load_7z_cue(&archive).unwrap();
        for (number, sectors) in [(2, 224), (3, 150), (25, 224)] {
            let track = loaded.disc.track(number).unwrap();
            assert_eq!(track.index1_lba() - track.stored_start_lba(), sectors);
        }
        let track = loaded.disc.track(3).unwrap();
        for lba in track.stored_start_lba()..track.index1_lba() {
            for sample in 0..588 {
                assert_eq!(loaded.disc.read_audio_sample(lba, sample).unwrap(), (0, 0));
            }
        }
        let program_probe = (track.index1_lba()..track.end_lba())
            .find_map(|lba| {
                (0..588).find_map(|sample| {
                    let pcm = loaded.disc.read_audio_sample(lba, sample).unwrap();
                    (pcm != (0, 0)).then_some((lba, sample, pcm))
                })
            })
            .expect("track 3 INDEX1 program contains no nonzero PCM sample");
        let subq_lba = track.index1_lba() - 1;

        let mut cd = CdRom2::new(loaded.disc.clone());
        let [_, high, middle, low] = subq_lba.to_be_bytes();
        send_command(&mut cd, &[0xD8, 0, 0, high, middle, low, 0, 0, 0, 0]);
        cd.advance_master_ticks(PROVISIONAL_CDROM2_PHASE_TICKS);
        ...
        let mut subq = [0; 10];
        for (index, byte) in subq.iter_mut().enumerate() {
            *byte = manual_read(&mut cd);
            if index != 9 {
                cd.advance_master_ticks(PROVISIONAL_CDROM2_NEXT_REQUEST_TICKS);
            }
        }
        let relative = track.index1_lba() - 1 - subq_lba;
        assert_eq!(subq[1], (track.control() << 4) | 1);
        assert_eq!((subq[2], subq[3]), (0x03, 0));
        assert_eq!(
            &subq[4..7],
            &[
                bcd(relative / (60 * 75)),
        ...
        println!(
            "track3_index0_lba={} index1_lba={} pregap_all_zero=true program_lba={} sample={} pcm={:?} subq={subq:02x?}",
            track.stored_start_lba(),
            track.index1_lba(),
            program_probe.0,
            program_probe.1,
            program_probe.2
