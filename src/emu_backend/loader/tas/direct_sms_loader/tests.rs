use anyhow::Result;
use zeff_emu_common::save_ram::SaveRamKind;

use super::*;
use crate::tas_project::{
    TasAutosaveConfig, TasAutosaveStore, TasControllerInput, TasEditorSession, TasInputFrame,
    TasSeekStateCache,
};
use crate::test_support::write_zip;

fn codemasters_rom() -> Vec<u8> {
    let offset = zeff_sega8_core::hardware::constants::CODEMASTERS_HEADER_OFFSET;
    let mut rom = vec![0xFF; offset + 16];
    rom[offset] = 2;
    rom[offset + 1..offset + 6].copy_from_slice(&[0x31, 0x08, 0x93, 0x10, 0x59]);
    rom[offset + 6..offset + 8].copy_from_slice(&0x1234u16.to_le_bytes());
    rom[offset + 8..offset + 10].copy_from_slice(&0xEDCCu16.to_le_bytes());
    rom[offset + 10..offset + 16].fill(0);
    rom
}

fn loader(
    label: &str,
) -> Result<(
    crate::test_support::TestDirectory,
    DirectSmsTasExecutionLoader,
    Vec<u8>,
)> {
    let directory = crate::test_support::test_directory(label)?;
    let path = directory.path().join("game.sms");
    let rom = codemasters_rom();
    std::fs::write(&path, &rom)?;
    Ok((directory, DirectSmsTasExecutionLoader::new(path), rom))
}

#[test]
fn creates_and_executes_two_pad_volatile_only_sms_project() -> Result<()> {
    let (directory, loader, rom) = loader("tas-direct-sms-isolated")?;
    let mut project = loader.create_project()?;
    assert_eq!(
        project.project_id(),
        format!(
            "sms-{}",
            crate::tas_project::TasDigest::from_bytes(&rom).to_hex()
        )
    );
    assert_eq!(project.identity().devices.len(), 2);
    assert_eq!(
        project.identity().sync_config_sha256,
        super::super::direct_sms::direct_sms_tas_sync_config_sha256()
    );
    assert!(super::super::validate_direct_sms_tas_project_identity(&project).is_ok());
    assert_eq!(
        super::super::classify_direct_tas_execution_profile(&project)?,
        crate::emu_thread::TasExecutionProfile::DirectSmsCartridge
    );

    let input = TasInputFrame {
        players: [
            TasControllerInput {
                buttons: 0x01,
                dpad: 0x04,
            },
            TasControllerInput {
                buttons: 0x02,
                dpad: 0x08,
            },
            TasControllerInput::default(),
            TasControllerInput::default(),
            TasControllerInput::default(),
        ],
        ..TasInputFrame::default()
    };
    project.edit_transaction(|edit| edit.set_input_range("main", 0, 1, input))?;
    let mut engine = loader.load_editor_engine(&project)?;
    let manual_path = directory.path().join("manual.ztas");
    let autosaves =
        TasAutosaveStore::beside_manual_save(&manual_path, TasAutosaveConfig::default())?;
    let cache = TasSeekStateCache::open(directory.path().join("seek-cache"))?;
    let mut editor = TasEditorSession::new(project, manual_path, autosaves, cache)?;
    let outcome = engine.seek(&mut editor, 1)?;

    let (mut expected, _) = loader.load_fresh_backend()?;
    expected.set_input(0x01, 0x04);
    expected.set_input_p2(0x02, 0x08);
    expected.step_frame();
    assert!(outcome.reached_target());
    assert_eq!(
        engine.backend().save_ram_kind(),
        SaveRamKind::KnownVolatile { size: 8 * 1024 }
    );
    assert_eq!(
        engine.backend().encode_state_bytes()?,
        expected.encode_state_bytes()?
    );
    Ok(())
}

#[test]
fn runtime_rejects_unknown_persistence_or_host_configuration() -> Result<()> {
    let (directory, loader, _) = loader("tas-direct-sms-runtime")?;
    let (backend, _) = loader.load_fresh_backend()?;
    assert!(super::super::validate_direct_sms_tas_runtime(&backend, false).is_ok());
    assert!(super::super::validate_direct_sms_tas_runtime(&backend, true).is_err());

    let unknown_path = directory.path().join("unknown.sms");
    let unknown = vec![0; 32 * 1024];
    std::fs::write(&unknown_path, &unknown)?;
    let load = |config| {
        crate::emu_backend::loader::load_backend_from_bounded_direct_source(
            ActiveSystem::MasterSystem,
            &unknown_path,
            unknown.clone(),
            config,
        )
        .map(|loaded| loaded.backend)
    };
    let unknown_backend = load(BackendLoadConfig {
        sega8_video_standard: Some(zeff_sega8_core::hardware::timing::Sega8VideoStandard::Ntsc),
        sega8_console_region: Some(zeff_sega8_core::hardware::region::Sega8Region::Export),
        ..BackendLoadConfig::default()
    })?;
    assert_eq!(
        unknown_backend.save_ram_kind(),
        SaveRamKind::MapperRamUnknown { size: 32 * 1024 }
    );
    assert!(super::super::validate_direct_sms_tas_runtime(&unknown_backend, false).is_err());

    let tagged_path = directory.path().join("game [mapper=korean].sms");
    let tagged_backend = crate::emu_backend::loader::load_backend_from_bounded_direct_source(
        ActiveSystem::MasterSystem,
        &tagged_path,
        codemasters_rom(),
        BackendLoadConfig::default(),
    )?
    .backend;
    assert_eq!(tagged_backend.save_ram_kind(), SaveRamKind::None);
    assert!(super::super::validate_direct_sms_tas_runtime(&tagged_backend, false).is_err());

    for config in [
        BackendLoadConfig {
            sample_rate: Some(44_100),
            ..BackendLoadConfig::default()
        },
        BackendLoadConfig {
            initial_input: Some((1, 0)),
            ..BackendLoadConfig::default()
        },
    ] {
        let backend = crate::emu_backend::loader::load_backend_from_bounded_direct_source(
            ActiveSystem::MasterSystem,
            &loader.source_path,
            codemasters_rom(),
            config,
        )?
        .backend;
        assert!(super::super::validate_direct_sms_tas_runtime(&backend, false).is_err());
    }
    Ok(())
}

#[test]
fn opening_refuses_changed_media_and_wrong_extension() -> Result<()> {
    let (directory, loader, mut rom) = loader("tas-direct-sms-identity")?;
    let project = loader.create_project()?;
    rom[0] ^= 1;
    std::fs::write(&loader.source_path, rom)?;
    assert!(loader.load_session(project.start_state()).is_err());

    let wrong = directory.path().join("game.zip");
    std::fs::write(&wrong, codemasters_rom())?;
    assert!(
        DirectSmsTasExecutionLoader::new(wrong)
            .create_project()
            .is_err()
    );
    Ok(())
}

#[test]
fn selected_sms_zip_member_binds_archive_member_and_effective_media() -> Result<()> {
    let directory = crate::test_support::test_directory("tas-direct-sms-zip")?;
    let archive_path = directory.path().join("games.zip");
    let first = codemasters_rom();
    let mut selected = first.clone();
    selected[0] ^= 1;
    let archive_bytes = write_zip(
        &archive_path,
        &[("first.sms", &first), ("folder/selected.sms", &selected)],
    )?;
    assert!(
        DirectSmsTasExecutionLoader::new_zip(archive_path.clone(), None)
            .create_project()
            .is_err()
    );
    let loader = DirectSmsTasExecutionLoader::new_zip(
        archive_path.clone(),
        Some(archive_path.join("folder/selected.sms")),
    );
    let project = loader.create_project()?;
    assert_eq!(
        project.identity().source_media_sha256,
        crate::tas_project::TasDigest::from_bytes(&archive_bytes)
    );
    assert_eq!(
        project.identity().effective_media_sha256,
        crate::tas_project::TasDigest::from_bytes(&selected)
    );
    assert_eq!(
        project.identity().sync_config_sha256,
        super::super::direct_sms::zip_sms_tas_sync_config_sha256("folder/selected.sms")
    );
    let reopened =
        DirectSmsTasExecutionLoader::new_zip_for_project(archive_path.clone(), &project)?;
    assert_eq!(
        reopened.load_session(project.start_state())?.identity(),
        project.identity()
    );

    write_zip(
        &archive_path,
        &[
            ("first.sms", &first),
            ("folder/selected.sms", &selected),
            ("note.txt", b"changed"),
        ],
    )?;
    assert!(DirectSmsTasExecutionLoader::new_zip_for_project(archive_path, &project).is_err());
    Ok(())
}

#[test]
fn replay_export_and_import_preserve_direct_sms_two_pad_input() -> Result<()> {
    let (directory, loader, _) = loader("tas-direct-sms-replay")?;
    let mut project = loader.create_project()?;
    let input = TasInputFrame {
        players: [
            TasControllerInput {
                buttons: 0x01,
                dpad: 0x04,
            },
            TasControllerInput {
                buttons: 0x02,
                dpad: 0x08,
            },
            TasControllerInput::default(),
            TasControllerInput::default(),
            TasControllerInput::default(),
        ],
        ..TasInputFrame::default()
    };
    project.edit_transaction(|edit| edit.set_input_range("main", 0, 1, input))?;
    let manual_path = directory.path().join("source.ztas");
    let autosaves =
        TasAutosaveStore::beside_manual_save(&manual_path, TasAutosaveConfig::default())?;
    let cache = TasSeekStateCache::open(directory.path().join("replay-cache"))?;
    let mut editor = TasEditorSession::new(project, manual_path, autosaves, cache)?;
    let plan = super::super::PrivateTasExecutionLoader::DirectSms(loader.clone());
    let replay_path = directory.path().join("movie.zrpl");
    plan.verify_and_export_editor_session(&mut editor, &replay_path)?;

    let imported_path = directory.path().join("imported.ztas");
    let imported = plan.import_replay_file(&replay_path, &imported_path, false)?;
    assert_eq!(imported.branch("main").unwrap().input_at(0), input);
    assert!(
        super::super::classify_direct_tas_execution_profile(&imported).is_ok_and(
            |profile| profile == crate::emu_thread::TasExecutionProfile::DirectSmsCartridge
        )
    );
    Ok(())
}
