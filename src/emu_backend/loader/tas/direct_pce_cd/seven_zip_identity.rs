...
pub(crate) fn arcade_multitap_sync_config_sha256() -> TasDigest {
    TasDigest::from_bytes(UNIQUE_ARCADE_MULTITAP_SYNC_CONFIGURATION)
}

pub(crate) fn memory_base_multitap_sync_config_sha256() -> TasDigest {
    TasDigest::from_bytes(UNIQUE_MEMORY_BASE_MULTITAP_SYNC_CONFIGURATION)
}
...
pub(crate) fn source_identity(
    ...
    hasher.update(b"zeff-tas-pce-cd-7z-source:v1\0");
...
pub(crate) fn ppf_source_identity(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"zeff-tas-pce-cd-7z-ppf-source:v1\0");
    hash_archive_ppf_source(
        &mut hasher,
        raw_source_sha256,
        raw_source_len,
...
pub(super) fn hash_archive_ppf_source(
    hasher: &mut Sha256,
    raw_source_sha256: [u8; 32],
    raw_source_len: usize,
    cue_member_path_sha256: [u8; 32],
    ...
    for (path, len, sha256) in patches {
