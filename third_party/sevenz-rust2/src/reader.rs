// Modified by Zeff Boy; see ../ZEFF_PATCH.md for the local delta.
...
use crate::{
    ...
    bitset::BitSet,
    block::*,
    decoder::{add_decoder, decoder_memory_usage_kb},
    error::Error,
};

const MAX_CODER_COUNT: usize = 32;
const MAX_MEM_LIMIT_KB: usize = 64 * 1024;

/// Resource limits applied while parsing and decoding an archive.
#[derive(Debug, Clone, Copy)]
pub struct ArchiveLimits {
    /// Maximum packed or decoded header size.
    pub max_header_bytes: usize,
    /// Maximum aggregate decoder working memory per folder.
    pub max_decoder_memory_kb: usize,
    /// Maximum file entries.
    pub max_files: usize,
    /// Maximum folders.
    pub max_blocks: usize,
    /// Maximum packed streams.
    pub max_pack_streams: usize,
    /// Maximum unpacked substreams.
    pub max_substreams: usize,
    /// Maximum coder layers in one folder.
    pub max_coders_per_block: usize,
    /// Maximum total input or output streams in one folder.
    pub max_streams_per_block: usize,
    /// Maximum bytes in one metadata property.
    pub max_property_bytes: usize,
    /// Maximum bytes in the names property.
...
impl Default for ArchiveLimits {
    fn default() -> Self {
        Self {
            ...
            max_decoder_memory_kb: 64 * 1024,
            max_files: 256,
            max_blocks: 256,
            max_pack_streams: 256,
            max_substreams: 256,
            max_coders_per_block: 32,
            max_streams_per_block: 256,
            max_property_bytes: 1024 * 1024,
            max_name_bytes: 1024 * 1024,
        }
    }
}

fn reserve_exact<T>(values: &mut Vec<T>, additional: usize, label: &str) -> Result<(), Error> {
    values
        .try_reserve_exact(additional)
        .map_err(|_| Error::other(format!("Unable to allocate {label}")))
}

fn zeroed_vec<T: Default + Clone>(len: usize, label: &str) -> Result<Vec<T>, Error> {
...
impl Archive {
    ...
    /// Reads archive metadata using explicit parser and decoder limits.
    ...
    fn read_header<R: Read + Seek>(
        ...
        if nid == K_ARCHIVE_PROPERTIES {
            Self::read_archive_properties(header, limit, limits)?;
            nid = header.read_u8()?;
        }
        ...
        if nid == K_MAIN_STREAMS_INFO {
            Self::read_streams_info(header, archive, limit, limits)?;
            nid = header.read_u8()?;
        }
        if nid == K_FILES_INFO {
            Self::read_files_info(header, archive, limit, limits)?;
            nid = header.read_u8()?;
        }
        if nid != K_END {
    ...
    fn read_archive_properties<R: Read + Seek>(
        ...
        while nid != K_END {
            ...
            let property_size = bounded_count(
                read_variable_u64(header)?,
                limit.min(limits.max_property_bytes),
                "propertySize",
            )?;
    ...
    fn init_archive<R: Read + Seek>(
        ...
        if start_header.next_header_size > usize::MAX as u64
            || start_header.next_header_size > limits.max_header_bytes as u64
        ...
        let mut buf = zeroed_vec(next_header_size_int, "next header")?;
        reader.read_exact(&mut buf)?;
        ...
        let mut header = if nid == K_ENCODED_HEADER {
            ...
            buf = Vec::new();
            reserve_exact(&mut buf, buf_size, "decoded header")?;
            (&mut out_reader)
        ...
        if nid == K_HEADER {
            Self::read_header(&mut header, &mut archive, header_len_bound, limits)?;
        } else {
    ...
    fn read_encoded_header<'r, R: Read, RI: 'r + Read + Seek>(
        ...
        let unpack_size = usize::try_from(block.get_unpack_size())
            .map_err(|_| Error::other("encoded header size exceeds this platform"))?;
        if unpack_size > limits.max_header_bytes {
            return Err(Error::MaxMemLimited {
                max_kb: limits.max_header_bytes / 1024,
                actaul_kb: unpack_size.div_ceil(1024),
            });
        }
        ...
        let mut decoder = if coder_len > 0 {
            let mut remaining_memory_kb = limits.max_decoder_memory_kb;
            for (index, coder) in block.ordered_coder_iter() {
                ...
                let memory_kb = decoder_memory_usage_kb(coder)?;
                if memory_kb > remaining_memory_kb {
                    return Err(Error::MaxMemLimited {
                        max_kb: limits.max_decoder_memory_kb,
                        actaul_kb: limits.max_decoder_memory_kb - remaining_memory_kb + memory_kb,
                    });
                }
                remaining_memory_kb -= memory_kb;
                let next = add_decoder(
                    decoder,
                    block.get_unpack_size_at_index(index) as usize,
                    coder,
                    password,
                    limits.max_decoder_memory_kb,
                    thread_count,
                )?;
    ...
    fn read_streams_info<R: Read>(
        ...
        let mut nid = header.read_u8()?;
        if nid == K_PACK_INFO {
            Self::read_pack_info(header, archive, limit, limits)?;
            nid = header.read_u8()?;
        }

        if nid == K_UNPACK_INFO {
            Self::read_unpack_info(header, archive, limit, limits)?;
            nid = header.read_u8()?;
        } else {
            archive.blocks.clear();
        }
        if nid == K_SUB_STREAMS_INFO {
            Self::read_sub_streams_info(header, archive, limit, limits)?;
            nid = header.read_u8()?;
        }
        if nid != K_END {
    ...
    fn read_files_info<R: Read + Seek>(
        ...
        let num_files = bounded_count(
            read_variable_u64(header)?,
            limit.min(limits.max_files),
            "num files",
        )?;
        let mut files: Vec<ArchiveEntry> = zeroed_vec(num_files, "files")?;
        ...
        loop {
            ...
            match prop_type {
                ...
                K_NAME => {
                    ...
                    let size =
                        bounded_count(size, limit.min(limits.max_name_bytes), "file names length")?;
    ...
    fn calculate_stream_map(archive: &mut Archive) -> Result<(), Error> {
        ...
        stream_map.block_first_pack_stream_index = zeroed_vec(num_blocks, "block pack indices")?;
        ...
        stream_map.pack_stream_offsets = zeroed_vec(num_pack_sizes, "pack offsets")?;
        ...
        stream_map.block_first_file_index = zeroed_vec(num_blocks, "block file indices")?;
        stream_map.file_block_index = zeroed_vec(archive.files.len(), "file block indices")?;
    ...
    fn read_pack_info<R: Read>(
        ...
        let num_pack_streams = bounded_count(
            read_variable_u64(header)?,
            limit.min(limits.max_pack_streams),
            "num pack streams",
        )?;
        let mut nid = header.read_u8()?;
        if nid == K_SIZE {
            archive.pack_sizes = zeroed_vec(num_pack_streams, "pack sizes")?;
        ...
        if nid == K_CRC {
            ...
            archive.pack_crcs = zeroed_vec(num_pack_streams, "pack CRCs")?;
    ...
    fn read_unpack_info<R: Read>(
        ...
        let num_blocks = bounded_count(
            read_variable_u64(header)?,
            limit.min(limits.max_blocks),
            "num blocks",
        )?;

        reserve_exact(&mut archive.blocks, num_blocks, "blocks")?;
        let external = header.read_u8()?;
        if external != 0 {
        ...
        for _ in 0..num_blocks {
            archive
                .blocks
                .push(Self::read_block(header, limit, limits)?);
        }
        ...
        for block in archive.blocks.iter_mut() {
            ...
            reserve_exact(&mut block.unpack_sizes, tos.min(limit), "unpack sizes")?;
            for _ in 0..tos {
    ...
    fn read_sub_streams_info<R: Read>(
        ...
        if nid == K_NUM_UNPACK_STREAM {
            ...
            for block in archive.blocks.iter_mut() {
                let num_streams = bounded_count(
                    read_variable_u64(header)?,
                    limit.min(limits.max_substreams),
                    "numStreams",
                )?;
                ...
                if total_unpack_streams > limit.min(limits.max_substreams) {
        ...
        reserve_exact(
            &mut sub_streams_info.unpack_sizes,
        ...
        sub_streams_info.crcs = zeroed_vec(total_unpack_streams, "substream CRCs")?;
        ...
        if nid == K_CRC {
            ...
            let mut missing_crcs = zeroed_vec(num_digests, "missing CRCs")?;
    ...
    fn read_block<R: Read>(
        ...
        let num_coders = bounded_count(
            read_variable_u64(header)?,
            limit.min(limits.max_coders_per_block.min(MAX_CODER_COUNT)),
            "num coders",
        )?;
        let mut coders = Vec::new();
        reserve_exact(&mut coders, num_coders, "coders")?;
        ...
        for _i in 0..num_coders {
            ...
            if total_in_streams > limits.max_streams_per_block as u64
                || total_out_streams > limits.max_streams_per_block as u64
            {
                return Err(Error::other("coder stream counts exceed configured limit"));
            }
            if coder.num_in_streams != 1 || coder.num_out_streams != 1 {
                return Err(Error::unsupported(
                    "Multi input/output coder graphs are unsupported",
                ));
            }
            if has_attributes {
                let properties_size = bounded_count(
                    read_variable_u64(header)?,
                    limit.min(limits.max_property_bytes),
                    "properties size",
                )?;
                let mut props = zeroed_vec(properties_size, "coder properties")?;
        ...
        reserve_exact(&mut bind_pairs, num_bind_pairs, "bind pairs")?;
        ...
        let mut packed_streams = zeroed_vec(num_packed_streams, "packed streams")?;
...
impl ArchiveReader<File> {
    ...
    pub fn open(path: impl AsRef<std::path::Path>, password: Password) -> Result<Self, Error> {
        Self::open_with_limits(path, password, ArchiveLimits::default())
    }

    /// Opens an archive using explicit parser and decoder limits.
    pub fn open_with_limits(
        ...
        Self::new_with_limits(file, password, limits)
    }
}
...
impl<R: Read + Seek> ArchiveReader<R> {
    ...
    pub fn new(source: R, password: Password) -> Result<Self, Error> {
        Self::new_with_limits(source, password, ArchiveLimits::default())
    }

    /// Creates a reader using explicit parser and decoder limits.
    ...
    fn build_decode_stack<'r>(
        ...
        let block = &archive.blocks[block_index];
        if block.coders.len() > limits.max_coders_per_block.min(MAX_CODER_COUNT) {
            return Err(Error::unsupported("Too many decoder layers"));
        }
        let mut remaining_memory_kb = limits.max_decoder_memory_kb;
        for (index, coder) in block.ordered_coder_iter() {
            if coder.num_in_streams != 1 || coder.num_out_streams != 1 {
    ...
    fn build_decode_stack2<'r>(
        ...
        reserve_exact(&mut sources, block.packed_streams.len(), "decoder inputs")?;
        ...
        R: 'r,
    {
        ...
        if coder.encoder_method_id() == EncoderMethod::ID_BCJ2 {
            ...
            reserve_exact(&mut inputs, num_in_streams, "decoder inputs")?;
...
mod limit_tests {
    ...
    #[test]
    fn coder_stream_limit_precedes_graph_allocations() {
        let bytes = [1, 0x11, 0, 0x81, 0x01, 1];
        let error = Archive::read_block(&mut Cursor::new(bytes), 1024, &ArchiveLimits::default())
            .unwrap_err();
