...
use liblzma::{
    read::XzDecoder,
    stream::{Filters as LzmaFilters, LzmaOptions, Stream as LzmaStream},
};
use lzma_rust2::{
...
pub fn add_decoder<I: Read>(
    ...
    match method.id() {
        ...
        EncoderMethod::ID_LZMA => {
            ...
            let mem_size = lzma_get_memory_usage_by_props(dict_size, coder.properties[0])
            ...
            let (lc, lp, pb) = decode_lzma_properties(coder.properties[0])?;
            let mut options =
                LzmaOptions::new_preset(6).map_err(|error| Error::other(error.to_string()))?;
            options
                .dict_size(dict_size)
                .literal_context_bits(lc)
                .literal_position_bits(lp)
                .position_bits(pb);
            let mut filters = LzmaFilters::new();
            filters.lzma1(&options);
            let stream = LzmaStream::new_raw_decoder(&filters)
                .map_err(|error| Error::other(error.to_string()))?;
            let lz = XzDecoder::new_stream(input, stream);
        ...
        EncoderMethod::LZMA => {
            ...
            Ok(
                lzma_get_memory_usage_by_props(get_lzma_dic_size(coder)?, coder.properties[0])
                    .map_err(|error| Error::other(error.to_string()))? as usize,
            )
        }
        EncoderMethod::LZMA2 => Ok(lzma2_get_memory_usage(get_lzma2_dic_size(coder)?) as usize),
        _ => Err(Error::UnsupportedCompressionMethod(
            method.name().to_string(),
        )),
    }
...
fn decode_lzma_properties(properties: u8) -> Result<(u32, u32, u32), Error> {
    let lc = u32::from(properties % 9);
    let remainder = u32::from(properties / 9);
    let lp = remainder % 5;
    let pb = remainder / 5;
    if pb > 4 {
        return Err(Error::Other("invalid LZMA properties byte".into()));
    }
    Ok((lc, lp, pb))
}
...
mod tests {
    ...
    #[test]
    fn lzma_and_lzma2_memory_estimators_enforce_limit_before_construction() {
        let lzma = coder(
            EncoderMethod::ID_LZMA,
            [vec![0x5D], (64_u32 * 1024 * 1024).to_le_bytes().to_vec()].concat(),
        );
        assert!(matches!(
            add_decoder(Cursor::new([]), 0, &lzma, &Password::empty(), 64 * 1024, 1),
            Err(Error::MaxMemLimited { .. })
        ));

        let lzma2 = coder(EncoderMethod::ID_LZMA2, vec![28]);
        assert!(matches!(
            add_decoder(Cursor::new([]), 0, &lzma2, &Password::empty(), 64 * 1024, 1),
            Err(Error::MaxMemLimited { .. })
        ));
    }

    #[test]
    fn lzma_properties_decode_and_reject_out_of_range_values() {
        assert_eq!(decode_lzma_properties(0x5D).unwrap(), (3, 0, 2));
        assert!(decode_lzma_properties(225).is_err());
    }

    #[test]
    fn compatibility_decoder_rejects_filters() {
        let delta = coder(EncoderMethod::ID_DELTA, vec![0]);
        assert!(matches!(
            add_decoder(Cursor::new([]), 0, &delta, &Password::empty(), 64 * 1024, 1),
