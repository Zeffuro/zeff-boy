...
impl Archive {
    ...
    /// Returns whether an entry is protected by an entry or folder CRC.
    pub fn entry_has_verifiable_crc(&self, index: usize) -> bool {
        self.files.get(index).is_some_and(|file| file.has_crc)
            || self
                .stream_map
                .file_block_index
                .get(index)
                ...
                .is_some_and(|block| block.has_crc)
    }
}
